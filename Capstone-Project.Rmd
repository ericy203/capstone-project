...
The below table gives us the total cases where players were correctly predicted 
```{r}
...
kable(data.frame(sens,spec,acc))
```
...
```{r}
tapply(predict_Test, college_draftTest$was_drafted,  FUN=mean)
```
...
## Recall
```{r}
ROCRperf_Test@x.values[[1]][1698]
```

## Precision
```{r}
ROCRperf_Test@y.values[[1]][1698]
```
...
## Recall
```{r}
ROCRperf_Test2@x.values[[1]][1613]
```

## Precision
```{r}
ROCRperf_Test2@y.values[[1]][1613]
```
